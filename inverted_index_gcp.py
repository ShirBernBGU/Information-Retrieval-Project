...
class MultiFileWriter:
    ...
    def __init__(self, base_dir, name, bucket_name=None):
        ...
        self._file_gen = (_open(str(self._base_dir / f'{name}_{i:03}.bin'), 
                                'wb', self._bucket) 
                          for i in itertools.count())  # generate the next bin number
        self._f = next(self._file_gen)  # _f saves the next file produced by the generator
    ...
    def write(self, b):
        ...
        while len(b) > 0:
            ...
            name = self._f.name if hasattr(self._f, 'name') else self._f._blob.name  # line added here not in colab version
...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            # f_name = str(self._base_dir / f_name)  # file in directory
            if f_name not in self._open_files:
                self._open_files[f_name] = _open(f_name, 'rb', self._bucket)
            f = self._open_files[f_name]
            f.seek(offset)  # move cursor to offset location
            n_read = min(n_bytes, BLOCK_SIZE - offset)  # read only relevant parts for each block
...
class InvertedIndex:  
    def __init__(self, docs={}):
        ...
        self.df = Counter()   # key - term: value - int (in how many documents this term has been in)
    ...
    def add_doc(self, doc_id, tokens):
        ...
        self.term_total.update(w2cnt)   # adding new doc requires updating global dictionary
        for w, cnt in w2cnt.items():
            self.df[w] = self.df.get(w, 0) + 1
            self._posting_list[w].append((doc_id, cnt))

    #   the same as _write_globals
    ...
    #  something about making is not serializable (the pickle)
    ...
    def read_a_posting_list(self, base_dir, w, bucket_name=None):
        posting_list = []
        if not w in self.posting_locs:    # word not in posting list
            return posting_list
        with closing(MultiFileReader(base_dir, bucket_name)) as reader:
            locs = self.posting_locs[w]    # get the location of the word w
            b = reader.read(locs, self.df[w] * TUPLE_SIZE)
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i*TUPLE_SIZE:i*TUPLE_SIZE+4], 'big')
                tf = int.from_bytes(b[i*TUPLE_SIZE+4:(i+1)*TUPLE_SIZE], 'big')
    ...
    @staticmethod
    def write_a_posting_list(b_w_pl, base_dir, bucket_name=None):    # b_w_pl --> (bucket_id, [(w0, posting_list_0), (w1, posting_list_1), ...])
        ...
        with closing(MultiFileWriter(base_dir, bucket_id, bucket_name)) as writer:
            ...
            path = str(Path(base_dir) / f'{bucket_id}_posting_locs.pickle')     #################### .pickle or .pkl extension
            bucket = None if bucket_name is None else get_bucket(bucket_name)
            with _open(path, 'wb', bucket) as f:
